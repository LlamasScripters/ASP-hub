...
function getRoomOpeningHours(room: SelectRoom, date: Date): { open: string, close: string } | null {
  const dayNames = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'] as const;
  const dayName = dayNames[date.getDay()];
...
function generateValidStartTime(room: SelectRoom, date: Date): Date | null {
  ...
  const randomOffsetMinutes = Math.floor(Math.random() * (durationMinutes - 59));
...
function generateValidDuration(room: SelectRoom, startDate: Date): number {
  ...
  const possibleDurations = [60, 90, 120, 180].filter(duration => duration <= maxDurationMinutes);
...
function generateReservationTitle(room: SelectRoom): string {
  ...
  const titles = activityTitles[sportType as keyof typeof activityTitles] || activityTitles['multi-sport'];
...
export async function seedReservations(
  ...
  if (existingUsers.length === 0) {
    console.log("No users found to assign as bookers. Skipping reservation creation.");
    return;
  }
  ...
  for (let dayOffset = 1; dayOffset <= 21; dayOffset++) {
    ...
    for (const room of rooms) {
      ...
      function hasOverlap(startA: Date, endA: Date, reservations: { start: Date, end: Date }[]): boolean {
        return reservations.some(existing =>
          (startA < existing.end && endA > existing.start)
        );
      }
  ...
  try {
    if (reservationsData.length > 0) {
      ...
      console.log(`${insertedReservations.length} reservations created successfully`);

      const statusCounts = insertedReservations.reduce((acc, reservation) => {
      ...
      console.log("No reservations generated (rooms closed or incompatible schedules)");
    }
  } catch (error) {
