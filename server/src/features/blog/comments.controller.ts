...
import { commentsService, type CreateCommentData, type UpdateCommentData } from "./comments.service.js";

const commentsRouter = Router();

// GET /api/comments/article/:articleId - Get all comments for an article
commentsRouter.get("/article/:articleId", async (req: Request, res: Response): Promise<void> => {
  try {
...
commentsRouter.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!articleId || !authorId || !content) {
      res.status(400).json({ error: "articleId, authorId et content sont requis" });
      return;
    }

    if (typeof content !== 'string' || content.trim().length === 0) {
      res.status(400).json({ error: "Le contenu du commentaire ne peut pas être vide" });
      return;
    }
...
commentsRouter.put("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (state !== undefined && !['published', 'archived'].includes(state)) {
      res.status(400).json({ error: "L'état doit être 'published' ou 'archived'" });
      return;
    }
...
commentsRouter.get("/article/:articleId/count", async (req: Request, res: Response): Promise<void> => {
  try {
    const { articleId } = req.params;
    const count = await commentsService.getCommentsCountByArticleId(articleId);
    res.json({ count });
  } catch (error) {
...
commentsRouter.get("/admin/article/:articleId", async (req: Request, res: Response): Promise<void> => {
  try {
    const { articleId } = req.params;
...
commentsRouter.put("/admin/:id/hide", async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const comment = await commentsService.updateComment(id, { state: 'archived' });
...
commentsRouter.put("/admin/:id/show", async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const comment = await commentsService.updateComment(id, { state: 'published' });
...
commentsRouter.get("/admin/all", async (req: Request, res: Response): Promise<void> => {
  try {
