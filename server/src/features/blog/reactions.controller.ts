...
reactionsRouter.get("/types", async (_req: Request, res: Response): Promise<void> => {
  try {
...
reactionsRouter.get("/articles/:articleId", async (req: Request, res: Response): Promise<void> => {
  try {
...
reactionsRouter.get("/comments/:commentId", async (req: Request, res: Response): Promise<void> => {
  try {
...
reactionsRouter.get("/articles/:articleId/user", async (req: Request, res: Response): Promise<void> => {
  try {
    const { articleId } = req.params;
    ...
    const reaction = await reactionsService.getUserArticleReaction(articleId, userId);
...
reactionsRouter.get("/comments/:commentId/user", async (req: Request, res: Response): Promise<void> => {
  try {
    const { commentId } = req.params;
    const session = await auth.api.getSession({
      headers: fromNodeHeaders(req.headers),
    ...
    const reaction = await reactionsService.getUserCommentReaction(commentId, userId);
    res.json({ success: true, data: reaction });
  } catch (error) {
...
reactionsRouter.post("/articles", async (req: Request, res: Response): Promise<void> => {
  try {
    const session = await auth.api.getSession({
      headers: fromNodeHeaders(req.headers),
    });
...
reactionsRouter.post("/comments", async (req: Request, res: Response): Promise<void> => {
  try {
    const session = await auth.api.getSession({
      headers: fromNodeHeaders(req.headers),
    });
