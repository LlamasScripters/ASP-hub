 ...
 */
sessionsRouter.get("/category/:categoryId", async (req: Request, res: Response) => {
	try {
		...
		console.error("Erreur lors de la récupération des sessions par catégorie:", error);
		res.status(500).json({ error: "Erreur serveur" });
	}
});
...
sessionsRouter.get("/coach/:coachId", async (req: Request, res: Response) => {
	try {
		...
		console.error("Erreur lors de la récupération des sessions par coach:", error);
		res.status(500).json({ error: "Erreur serveur" });
	}
});
 ...
 */
sessionsRouter.post("/", requireSessionManagement(), async (req: Request, res: Response) => {
	try {
		const result = createSessionSchema.safeParse(req.body);
		...
		if (error instanceof Error && error.message.includes("Conflits détectés")) {
 ...
 */
sessionsRouter.put("/:id", requireSessionManagement(), async (req: Request, res: Response) => {
	try {
		const { id } = req.params;
 ...
 */
sessionsRouter.delete("/:id", requireSessionManagement(), async (req: Request, res: Response) => {
	try {
		const { id } = req.params;
 ...
 */
sessionsRouter.patch("/:id/status", requireSessionManagement(), async (req: Request, res: Response) => {
	try {
		const { id } = req.params;
		const { status, notes } = req.body;
		
		const statusSchema = z.enum(["planifie", "en_cours", "termine", "annule"]);
