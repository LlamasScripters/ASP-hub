...
const baseSessionSchema = z.object({
	...
	description: z
		...
		.optional(),
	type: z
		.enum(["entrainement", "match", "stage", "competition", "autre"], {
	...
	maxParticipants: z
		.number()
		.int()
		.min(1, "Le nombre maximum de participants doit être positif")
		.optional(),
...
export const createSessionSchema = baseSessionSchema
	.refine(
		(data) => {
	...
	.refine(
		...
		{
			message:
				"La date de début doit être dans le futur pour les sessions planifiées",
			path: ["startDate"],
		},
	);

// Schema pour la modification d'une session
export const updateSessionSchema = baseSessionSchema
	.partial()
