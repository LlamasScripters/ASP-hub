 ...
 */
responsibilitiesRouter.post("/sections/:sectionId/manager", 
	...
	async (req: Request, res: Response) => {
		try {
			...
			res.status(500).json({ error: "Erreur lors de l'assignation du responsable" });
		}
	}
);
 ...
 */
responsibilitiesRouter.delete("/sections/:sectionId/manager", 
	requireRole(UserRole.ADMIN), 
	async (req: Request, res: Response) => {
		try {
			const { sectionId } = req.params;
 ...
 */
responsibilitiesRouter.get("/sections/:sectionId", async (req: Request, res: Response) => {
	try {
		const { sectionId } = req.params;
		
		const responsibilities = await responsibilitiesService.getSectionResponsibilities(sectionId);
		
		res.json(responsibilities);
	} catch (error) {
		console.error("Erreur lors de la récupération des responsabilités:", error);
 ...
 */
responsibilitiesRouter.post("/categories/:categoryId/coach", 
	requireRole(UserRole.ADMIN), 
	async (req: Request, res: Response) => {
		try {
 ...
 */
responsibilitiesRouter.delete("/categories/:categoryId/coach", 
	requireRole(UserRole.ADMIN), 
	async (req: Request, res: Response) => {
		try {
			const { categoryId } = req.params;
 ...
 */
responsibilitiesRouter.get("/categories/:categoryId", async (req: Request, res: Response) => {
	try {
		const { categoryId } = req.params;
		
		const responsibilities = await responsibilitiesService.getCategoryResponsibilities(categoryId);
 ...
 */
responsibilitiesRouter.get("/users/:userId", async (req: Request, res: Response) => {
	try {
		const { userId } = req.params;
		
		const responsibilities = await responsibilitiesService.getUserResponsibilities(userId);
...
responsibilitiesRouter.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const users = await responsibilitiesService.getEligibleUsersForSection(sectionId);
			res.json(users);
		} catch (error) {
			console.error("Erreur getEligibleUsersForSection:", error);
			res.status(500).json({
...
responsibilitiesRouter.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const users = await responsibilitiesService.getEligibleUsersForCategory(categoryId);
			res.json(users);
		} catch (error) {
			console.error("Erreur getEligibleUsersForCategory:", error);
			res.status(500).json({
