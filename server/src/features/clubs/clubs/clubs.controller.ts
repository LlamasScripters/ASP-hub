 ...
 */
clubsRouter.get("/:id", requireMemberAccess(), async (req: Request, res: Response) => {
	try {
 ...
 */
clubsRouter.post("/", requireRole(UserRole.ADMIN), async (req: Request, res: Response) => {
	try {
 ...
 * Accessible aux administrateurs uniquement
 */
clubsRouter.put("/:id", requireRole(UserRole.ADMIN), async (req: Request, res: Response) => {
	try {
		const params = clubParamsSchema.safeParse(req.params);
		if (!params.success) {
			res.status(400).json({ error: params.error.flatten() });
 ...
 * Accessible aux administrateurs uniquement
 */
clubsRouter.delete("/:id", requireRole(UserRole.ADMIN), async (req: Request, res: Response) => {
	try {
		const params = clubParamsSchema.safeParse(req.params);
		if (!params.success) {
			res.status(400).json({ error: params.error.flatten() });
