...
function validateAgainstParent(
  complexSchedule: Record<string, { open: string|null; close: string|null; closed: boolean }>,
  roomSchedule: Record<string, { open: string|null; close: string|null; closed: boolean }>
) {
  for (const day of Object.keys(complexSchedule)) {
	...
	if (p.closed && !c.closed) {
	  throw new Error(`The complex is closed on ${day}, the room must also be closed`);
	}
	if (!p.closed && !c.closed) {
	  if ((c.open !== null && p.open !== null && c.open < p.open) || (c.close !== null && p.close !== null && c.close > p.close)) {
		throw new Error(
...
export const roomsService = {
	...
	update: async (
		...
		const complexId = data.complexId ?? (await db.select().from(rooms).where(eq(rooms.id,id)))[0].complexId;
