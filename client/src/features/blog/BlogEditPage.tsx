...
export default function BlogEditPage({ blog }: BlogEditPageProps) {
  ...
  const createNewTag = () => {
    if (newTagName.trim() && !availableTags.find(tag => tag.name.toLowerCase() === newTagName.trim().toLowerCase())) {
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <CardDescription>Modifiez les informations de cet article</CardDescription>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-muted/50 rounded-lg">
                  <h4 className="text-sm font-medium mb-2">Informations article</h4>
                  <div className="text-xs space-y-1 text-muted-foreground">
                    <p>Créé: <span className="font-medium">{formatDate(blog.createdAt)}</span></p>
                    <p>Modifié: <span className="font-medium">{formatDate(blog.updatedAt)}</span></p>
                    {blog.publishedAt && (
                      <p>Publié: <span className="font-medium">{formatDate(blog.publishedAt)}</span></p>
                    )}
                    <p>Auteur ID: <span className="font-medium">{blog.authorId}</span></p>
                  </div>
                </div>
              </div>
            </div>
