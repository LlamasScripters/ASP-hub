  ...
  const filteredAndSortedBlogs = useMemo(() => {
    let filtered = blogs.filter(blog => {
      ...
      const matchesTag = selectedTag === "all" || 
        (blog.tags && blog.tags.some(tag => tag.name === selectedTag));
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <p className="text-gray-600 dark:text-gray-400">
            {filteredAndSortedBlogs.length} article{filteredAndSortedBlogs.length > 1 ? 's' : ''} trouvé{filteredAndSortedBlogs.length > 1 ? 's' : ''}
            {searchTerm && (
        ...
        {filteredAndSortedBlogs.length === 0 ? (
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredAndSortedBlogs.map((blog) => (
              <Card key={blog.id} className="group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 bg-white dark:bg-slate-800 border-0 shadow-lg">
                {/* Image d'en-tête */}
                {blog.headerImage && (
