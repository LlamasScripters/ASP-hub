...
// @ts-ignore
import { Info, Calendar as CalendarIcon, Loader2, Clock } from "lucide-react";
...
export function ReservationForm({
    ...
    const validateReservationTime = useCallback((startAt: Date, endAt: Date): string | null => {
        ...
        if (!daySchedule || daySchedule.closed) {
            const dayName = daysOfWeek.find(d => d.key === dayKey)?.label || "ce jour";
        ...
        if (!isValidTimeSlot) {
            const scheduleText = daySchedule.open && daySchedule.close ? `${daySchedule.open} - ${daySchedule.close}` : 'Non défini';
            const dayName = daysOfWeek.find(d => d.key === dayKey)?.label || "ce jour";
            ...
            <CardContent>
                ...
                <div className="mb-6 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                        {daysOfWeek.map((day) => {
                            const schedule = roomOpeningHours[day.key as keyof typeof roomOpeningHours];
                            return (
                ...
                <Form {...form}>
                    <form
                        onSubmit={async (e) => {
                            ...
                            if (startAt && endAt) {
                                const timeValidationError = validateReservationTime(startAt, endAt);
