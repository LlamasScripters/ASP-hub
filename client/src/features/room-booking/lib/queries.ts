...
import { complexesApi, roomsApi, roomReservationsApi } from "@room-booking/lib/api";
import type { 
  ComplexFilters
} from "@/features/room-booking/hooks/useComplexes";
import type { 
  RoomFilters
} from "@/features/room-booking/hooks/useRooms";
...
export const roomReservationQueries = {
  ...
  byRoom: (roomId: string, startDate: Date, endDate: Date) =>
    queryOptions({
      queryKey: [...roomReservationQueries.lists(), 'by-room', roomId, startDate, endDate],
      queryFn: () => roomReservationsApi.getRoomReservationsByRoomId(roomId, startDate, endDate),
