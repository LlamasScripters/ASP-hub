...
export function RoomsPage({ initialRooms = [] }: RoomsPageProps) {
	...
	const roomStats = useMemo(() => {
		...
		const sportCounts = initialRooms.reduce(
			(acc, room) => {
	...
	const availableSports = useMemo(() => {
		return Array.from(
			new Set(initialRooms.map((room) => room.sportType)),
		).sort();
	}, [initialRooms]);
	...
	const filteredAndSortedRooms = useMemo(() => {
		const filtered = initialRooms.filter((room) => {
			...
			const matchesSport =
				filterSport === "all" || room.sportType === filterSport;
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{mockRoomStats.totalReservations}
						</div>
			...
			<Tabs defaultValue="rooms" className="space-y-4">
				...
				<TabsContent value="rooms" className="space-y-4">
								...
								<div className="flex items-center gap-2 mt-4 text-sm text-muted-foreground">
									...
									{filterType !== "all" && (
										<Badge variant="secondary">
											Type:{" "}
											{filterType === "indoor" ? "Intérieure" : "Extérieure"}
										</Badge>
									)}
									{filterSport !== "all" && (
					...
					<div className="text-sm text-muted-foreground mb-4">
						...
						{filteredAndSortedRooms.length !== roomStats.total &&
							` sur ${roomStats.total} au total`}
					</div>
								...
								<Card className="cursor-pointer hover:shadow-lg transition-all duration-200 hover:border-primary/20">
									<CardHeader className="pb-3">
										<div className="flex items-start justify-between">
											...
											<Badge
												variant="outline"
												className={getSportBadgeColor(room.sportType)}
											>
												{room.sportType}
											</Badge>
										</div>
									...
									<CardContent className="pt-0">
										<div className="space-y-3">
											<div className="flex items-center justify-between text-sm">
												...
												<Badge
													variant={room.isIndoor ? "default" : "secondary"}
												>
