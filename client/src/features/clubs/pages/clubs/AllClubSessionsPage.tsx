...
import { ArrowLeft, Calendar, Users, ChevronUp, ChevronDown, Search, Filter, Edit, Trash2, Clock, MapPin } from "lucide-react";
import { toast } from "sonner";
import type { Section, Category, SessionSport } from "../../types";

type SortField = 'title' | 'sectionName' | 'categoryName' | 'type' | 'status' | 'startDate';
...
export function AllClubSessionsPage() {
  ...
  const [deleteSession, setDeleteSession] = useState<EnrichedSession | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const sectionsData: Section[] = await fetch(`/api/clubs/${clubId}/sections`).then(res => {
          if (!res.ok) throw new Error('Erreur lors du chargement des sections');
          return res.json();
        });
        ...
        for (const section of sectionsData) {
          const categories: Category[] = await fetch(`/api/clubs/${clubId}/sections/${section.id}/categories`).then(res => {
            if (!res.ok) throw new Error('Erreur lors du chargement des catégories');
            return res.json();
          });
          
          for (const cat of categories) {
            const catSessions: SessionSport[] = await fetch(`/api/clubs/${clubId}/sections/${section.id}/categories/${cat.id}/sessions`).then(res => {
              if (!res.ok) throw new Error('Erreur lors du chargement des sessions');
              return res.json();
            });
            result.push(...catSessions.map((s) => ({ 
              ...s, 
  ...
  const filteredAndSortedSessions = useMemo(() => {
    let filtered = sessions.filter(session => {
      const titleMatch = session.title.toLowerCase().includes(filters.title.toLowerCase());
      const sectionMatch = filters.sectionName === '' || 
        (session.sectionName && session.sectionName.toLowerCase().includes(filters.sectionName.toLowerCase()));
      const categoryMatch = filters.categoryName === '' || 
        (session.categoryName && session.categoryName.toLowerCase().includes(filters.categoryName.toLowerCase()));
      ...
      return titleMatch && sectionMatch && categoryMatch && typeMatch && statusMatch;
    });
  ...
  const upcomingSessions = sessions.filter(s => new Date(s.startDate) > new Date() && s.status === 'planifie').length;
        ...
        {sessions.length === 0 ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {filteredAndSortedSessions.length} session{filteredAndSortedSessions.length > 1 ? 's' : ''} affichée{filteredAndSortedSessions.length > 1 ? 's' : ''} sur {sessions.length} au total
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Filtres */}
                      ...
                      <SelectContent>
                        ...
                        {uniqueTypes.map(type => (
                          <SelectItem key={type} value={type}>
                            {type === 'entrainement' ? 'Entraînement' :
                             type === 'match' ? 'Match' :
                             type === 'stage' ? 'Stage' :
                             type === 'competition' ? 'Compétition' : 'Autre'}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                            ...
                            <div className="space-y-1">
                              <div>
                                {new Date(s.startDate).toLocaleDateString('fr-FR', {
                                  ...
                                  month: '2-digit',
                                  year: 'numeric'
                                })} à {new Date(s.startDate).toLocaleTimeString('fr-FR', {
                                  hour: '2-digit',
                                  minute: '2-digit'
                                })}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                → {new Date(s.endDate).toLocaleDateString('fr-FR', {
                                  ...
                                  month: '2-digit',
                                  year: 'numeric'
                                })} à {new Date(s.endDate).toLocaleTimeString('fr-FR', {
                                  hour: '2-digit',
                                  minute: '2-digit'
                                })}
                              </div>
      ...
      <AlertDialog open={!!deleteSession} onOpenChange={() => setDeleteSession(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la session <strong>"{deleteSession?.title}"</strong> ?
              <br /><br />
              ...
              <ul className="list-disc list-inside mt-2 space-y-1">
                <li>La perte de tous les participants inscrits à cette session</li>
                <li>La suppression de l'historique des présences</li>
                <li>La perte de toutes les données statistiques liées à cette session</li>
