...
export function CategoriesListPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const sectionRes = await fetch(`/api/clubs/${clubId}/sections/${sectionId}`);
  ...
  const handleDeleteCategory = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/clubs/${clubId}/sections/${sectionId}/categories/${deleteCategory.id}`, {
        method: 'DELETE',
      });
    ...
    <div className="container mx-auto p-6 space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {filteredAndSortedCategories.length} catégorie{filteredAndSortedCategories.length > 1 ? 's' : ''} affichée{filteredAndSortedCategories.length > 1 ? 's' : ''} sur {categories.length} au total
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filtres */}
          <div className="mb-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-medium text-muted-foreground">Nom de la catégorie</label>
                <Input
      ...
      <AlertDialog open={!!deleteCategory} onOpenChange={() => setDeleteCategory(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la catégorie <strong>"{deleteCategory?.name}"</strong> ?
              <br /><br />
