...
import { ArrowLeft, Users, Edit, Calendar, Tag, FolderOpen, Plus, Trash2 } from "lucide-react";
...
export function AllCategoriesPage() {
  ...
  const [deleteCategory, setDeleteCategory] = useState<EnrichedCategory | null>(null);
    ...
    const fetchAll = async () => {
      try {
        ...
        for (const section of sectionsData) {
          const cats: Category[] = await fetch(`/api/clubs/${clubId}/sections/${section.id}/categories`).then(res => {
  ...
  const handleDeleteCategory = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/clubs/${clubId}/sections/${deleteCategory.sectionId}/categories/${deleteCategory.id}`, {
        method: 'DELETE',
      });
  ...
  const avgAge = categories.length > 0 
    ? Math.round(categories.reduce((acc, cat) => {
        const avg = cat.ageMin && cat.ageMax ? (cat.ageMin + cat.ageMax) / 2 : 0;
        return acc + avg;
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{avgAge > 0 ? `${avgAge} ans` : "N/A"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-orange-50 border-orange-200 dark:bg-orange-950/20 dark:border-orange-800">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Avec âge défini</p>
                  <p className="text-2xl font-bold">
                    {categories.filter(cat => cat.ageMin || cat.ageMax).length}
                  </p>
                </div>
              </div>
            </CardContent>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div 
                              ...
                              style={{ backgroundColor: cat.sectionColor || "#3b82f6" }}
                            />
