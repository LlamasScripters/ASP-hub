...
export function BlogCommentsAdmin({ blogId }: BlogCommentsAdminProps) {
  ...
  const toggleCommentVisibility = async (commentId: string, currentState: string) => {
    try {
      setUpdatingComments(prev => new Set(prev).add(commentId));
      
      const action = currentState === 'published' ? 'hide' : 'show';
      const response = await fetch(`/api/comments/admin/${commentId}/${action}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Erreur lors du ${action === 'hide' ? 'masquage' : 'affichage'} du commentaire`);
      }
      ...
      setComments(prev => 
        prev.map(comment => 
          comment.id === commentId 
            ? { ...comment, state: action === 'hide' ? 'archived' : 'published' }
            : comment
        )
      );
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-3 text-muted-foreground">Chargement des commentaires...</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardHeader>
        ...
        <div className="flex gap-2">
          <Badge variant="outline" className="gap-1">
            ...
            {publishedComments.length} visible{publishedComments.length > 1 ? 's' : ''}
          </Badge>
          {hiddenComments.length > 0 && (
            <Badge variant="secondary" className="gap-1">
              <EyeOff className="h-3 w-3" />
              {hiddenComments.length} masqué{hiddenComments.length > 1 ? 's' : ''}
            </Badge>
          )}
        </div>
      </CardHeader>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge 
                        variant={comment.state === 'published' ? 'default' : 'secondary'}
                        className="text-xs"
                      >
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant={comment.state === 'published' ? 'destructive' : 'default'}
                        size="sm"
                        onClick={() => toggleCommentVisibility(comment.id, comment.state)}
        ...
        {hiddenComments.length > 0 && (
          <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <p className="text-xs text-amber-700">
              {hiddenComments.length} commentaire{hiddenComments.length > 1 ? 's sont masqués' : ' est masqué'} et 
              ne {hiddenComments.length > 1 ? 'sont' : 'est'} pas visible{hiddenComments.length > 1 ? 's' : ''} 
