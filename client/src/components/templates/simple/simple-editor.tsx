...
export function SimpleEditor({ value, onChange }: SimpleEditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose min-h-[200px] max-w-none border rounded-md p-3 bg-background focus:outline-none',
      },
    },
  });
  ...
  if (!editor) {
    return <div className="min-h-[200px] border rounded-md p-3 bg-muted animate-pulse">Chargement de l'éditeur...</div>;
  }
    ...
    <div className="w-full">
      <div className="border-b mb-2 pb-2 flex gap-1 flex-wrap">
        ...
        <button 
          ...
          className={`px-3 py-1 text-sm border rounded hover:bg-gray-100 ${
            editor.isActive('strike') ? 'bg-blue-100 text-blue-700 line-through' : ''
          }`}
        >
          S
        </button>
